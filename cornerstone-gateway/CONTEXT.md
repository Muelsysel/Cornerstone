...
- 令牌→上下文头透传：`sub`→`X-Cornerstone-User-Id`、`username/preferred_username`→`X-Cornerstone-Username`、`deptId`→`X-Cornerstone-Dept-Id`、角色→`X-Cornerstone-Roles`（优先 JWT 的 `roles` 声明，client_credentials 无 roles 时兜底 `scope`，逗号连接）。头名与 `UserContext` 约定一致；入口统一剥除外部伪造的同名头与 `X-Internal-Token`，转发时盖章 `X-Internal-Token`（证明经网关转发，下游 `UserContextFilter` 仅采信有效令牌的透传头）。
