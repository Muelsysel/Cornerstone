...
- fix(ux): 菜单管理**按钮节点不可作为父级**——父级树选择器过滤按钮(F)节点（叶子权限点），按钮行隐藏「新增」按钮（与后端 `validateParent` 校验闭环）

**测试方法**：`npm test`（34 用例）+ `npm run build`。

## [1.4.44] - 2026-08-16

- feat(dict): 字典数据项弹窗加**标签模糊搜索**（dictLabel，后端早已支持前端无入口）+ 重置

**测试方法**：`npm test`（34 用例）+ `npm run build`。

## [1.4.43] - 2026-08-16

- feat(ux): 用户管理搜索加**部门筛选**（树选择器，复用表单部门树）

**测试方法**：`npm test`（34 用例）+ `npm run build`。

## [1.4.42] - 2026-08-16

- feat(ux): 个人资料弹窗「部门」显示部门名（后端 profile 回填 deptName），不再裸显部门 ID

**测试方法**：`npm test`（34 用例）+ `npm run build`。

## [1.4.41] - 2026-08-16

- feat(config): 参数列表加「内置」筛选（系统/自定义，后端 configType 过滤参数此前无前端入口）
...
- feat(dict): 字典数据项表单补「是否默认」（isDefault Y/N）字段 + 列表加默认标记列——此前后端支持但前端无入口，默认项无法通过 UI 维护

**测试方法**：`npm test`（34 用例）+ `npm run build`。

## [1.4.37] - 2026-08-16

- feat(ux): 侧边栏可折叠——顶栏新增折叠开关（窄屏/专注模式让出内容区），logo 文字折叠时隐藏，宽度过渡 220px ↔ 64px

**测试方法**：`npm test`（34 用例）+ `npm run build`。

## [1.4.36] - 2026-08-16

- fix(contract): 用户页移除无效的手机号/邮箱字段（后端 `sys_user` 表无 phone/email 列，提交值被静默丢弃）；改为**部门树选择器**（`/system/dept/tree`，数据权限范围依赖 deptId 归属）；个人资料弹窗同步去掉手机号/邮箱、补部门ID；用户备注补 maxlength=500；公告表单标题/内容补字数统计

**测试方法**：`npm test`（34 用例）+ `npm run build`。

## [1.4.35] - 2026-08-16

- feat(ux): 公告表单标题/内容字数统计（show-word-limit，与后端 100/20000 上限对齐）
...
- fix(ux): 表单输入加 maxlength 对齐后端列长——用户（用户名/昵称 30）、角色（名称 30/标识 50）、部门（名称 50）、公告（标题 100/内容 20000）；输入即限制，避免后端 400 拒绝
