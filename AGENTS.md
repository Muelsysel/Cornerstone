...
- 技术栈：Java 17 · Spring Boot 3.2 · Spring Cloud 2023 · Spring Cloud Alibaba (Nacos) · Spring Cloud Gateway · OpenFeign · Spring Security + OAuth2 · MyBatis-Plus · Redis · MySQL
- 模块：`cornerstone-gateway`（网关）· `cornerstone-auth`（认证中心）· `cornerstone-system`（系统服务）· `cornerstone-common`（公共模块）· `cornerstone-api`（跨服务 Feign 契约）
