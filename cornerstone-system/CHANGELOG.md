...
**测试方法**：`mvn test -pl cornerstone-system`（165 用例）。
...
**测试方法**：`mvn test -pl cornerstone-system`（163 用例）。
...
**测试方法**：`mvn test -pl cornerstone-system`（160 用例）。

## [1.2.47] - 2026-08-16

- feat(log): 操作/登录日志分页支持**时间区间过滤**（beginTime/endTime → oper_time/login_time ge/le）；两个 ServiceImplTest 新增时间区间条件断言

**测试方法**：`mvn test -pl cornerstone-system`（159 用例）。
...
**测试方法**：`mvn test -pl cornerstone-system`（156 用例）。
