...
- test: 后端 246→**266 用例**（common 35 + gateway 21 + auth 19 + system 164 + demo 27）、前端 34、e2e 34→**37**；verify-chain 多轮稳定 PASS
...
## [1.5.20] - 2026-08-15（端到端契约扩展）

- feat(scripts): `verify-chain.ps1` 断言 16→17 项——新增部门树查询断言；公告发布/下线断言改"操作响应 + 状态兜底确认"（消除本机 curl 偶发假失败，3 次连跑稳定 PASS）；run-demo/README 同步

## [1.5.19] - 2026-08-15（端到端契约扩展）

- feat(scripts): `verify-chain.ps1` 断言 15→16 项——新增菜单树查询断言（admin 可获取完整菜单树，RBAC 只读契约回归）；run-demo/README 同步
...
## [1.5.16] - 2026-08-15（README 同步）

- docs: README ADR 数量 8→9、端到端行引用 verify-chain 13 项断言
