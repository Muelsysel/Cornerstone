...
### 3. 工程规范（遵循项目既有模式，避免返工）

- **接口签名以 `cornerstone-api` 与 OpenAPI 文档为准**，不确定先查阅再动手
- **需求含糊先确认，不自行脑补**；业务规则以 CONTEXT.md 词汇表为准
- **复用现有契约**（common 的 Result/错误码/异常、api 的 Feign 接口），不另起炉灶
- **每个功能带测试**（见步骤 5）；**文档与代码同改**（见步骤 6 的文档维护义务）
...
- **文档维护义务（强制）**：修改任何模块，必须同步：
  1. 该模块 `CHANGELOG.md` 顶部新增条目（版本/日期/类型/说明/涉及接口与测试）
  2. 该模块 `CONTEXT.md`（职责/边界/词汇表/测试方法如有变化）
  3. 按该模块 `CONTEXT.md` 的"测试方法"章节全量跑测试，通过后再提交
- 中文描述，一句话说清"改了什么、为什么"
...
无论哪种工具，`AGENTS.md` 都是入口——先让它读到黄金法则与文档维护义务，再让 AI 加载本技能。
