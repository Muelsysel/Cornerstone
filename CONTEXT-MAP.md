...
| **用户登录**（Login） | `cornerstone-auth` 的 `POST /auth/login`：用户名密码（BCrypt）校验后签发带角色与权限（scope）的 JWT；防爆破双层防护——网关 IP 限流 + 账号级锁定（连续失败 ≥5 次锁 5 分钟，见 ADR-0009）；测试账号 admin/admin123。 |
